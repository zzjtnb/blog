...
description: 'Lua中的package.path和package.cpath 很多LUA的模块：包含C动态库和lua封装代码两部分. C动态库必须放在C库路径下，Lua封装代码需放在Lua库路径下. 可以通过以下命令来查看C库路径和Lua库路径： pac'
date: '2021-03-25 20:30:26'
updated: '2021-03-25 20:32:18'
category: 'Lua'
tags:
  - 'Lua'
...
hot: false
sourceId: 225
sourcePath: 'luazdpackagepathhpackagecpath'
---
# Lua中的package.path和package.cpath
>很多LUA的模块：包含C动态库和lua封装代码两部分.
C动态库必须放在C库路径下，Lua封装代码需放在Lua库路径下.
可以通过以下命令来查看C库路径和Lua库路径：
```lua
