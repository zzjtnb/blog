...
description: '关于给input、div添加hover样式，边框变大后里面的文字移动问题解决办法 这样写，鼠标移上去后input里的文字就会移动 解决办法，给要增大边框先预留一个空间，可以input添加一个padding：1px; 然后鼠标以上去在设置为0'
...
hot: false
sourceId: 243
sourcePath: 'ginputdivtjhoverysbkbdhlmdwzydwtjjbf'
---
...
解决办法，给要增大边框先预留一个空间，可以`input`添加一个`padding：1px`;  然后鼠标以上去在设置为0，`input:hover{padding:0px}`;
```html
<style>
  div {
    width: 200px;
